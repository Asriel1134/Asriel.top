<script setup>
    ...
    const emit = defineEmits(['msg', 'returnFromNoteEditer'])
    const note = ref({
        title: '',
        description: '',
        text: '',
    ...
    function addNote() {
        if (note.value.className != '' && note.value.title != ''){
            axios({
                method: 'post',
                url: globalProperties.$httpUrl + "/addNote",
                params: {
                ...
                emit('returnFromNoteEditer')
            })
        } else {
            if (note.value.className != ''){
            emit('msg', "Please choose a classification")
            }
            if (note.value.title != ''){
            emit('msg', "Please input a title")
            }
        }
    }
    ...
    <div class="noteEditer">
        <div class="noteInfo">
            ...
            <div class="submitArea">
                <button class="submit" @click="addNote">
                    Submit
                </button>
